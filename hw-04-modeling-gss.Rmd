...
gss16 <- gss16 %>%
  mutate(advfront = case_when(
    str_detect(advfront, "(?i)^(Strongly agree|Agree)$") ~ "agree",
    TRUE ~ "Not agree"
  ))

gss16$advfront <- factor(gss16$advfront, levels = c("agree", "Not agree"))
```
...
***`Agree` has 1159 answers while `Not agree` has 1708 answers***
...
```{r}
gss16 <- gss16 %>%
  mutate(polviews = case_when(str_detect(polviews, "(?i)liberal") ~ "Liberal",
                              str_detect(polviews, "(?i)conservative") ~ "Conservative",
                              TRUE ~ polviews))

gss16$polviews <- fct_relevel(gss16$polviews, "Conservative", "Moderate", "Liberal")
```
...
gss16_rec_2 <- recipe(advfront ~ polviews + educ, data = gss16_train) %>%
  step_other(polviews, threshold = 0.10, other = "Other") %>%
  step_dummy(all_nominal(), -all_outcomes())
...
***The model excluding `wrkstat` (model 2) performs slightly better than model 1***
...
```{r}
harrass_subset <- gss16 %>%
  filter(harass5 %in% c("Yes","No"))

count(harrass_subset, harass5)
```

***1136 people fortunately said No while 237 people unfortunately said Yes***
...
***Bootstrapping displays averages which can apply to Americans as a whole (the whole country essentially)***
...
```{r}
harass_bootstrapping <- c()
i <- 1
for(i in c(1:500)) {
  harVar <- harrass_subset %>%
 slice_sample(n = 1373, replace = TRUE) %>%
  count(harass5) %>%
  mutate(n = n/sum(n)) %>%
  slice_tail()

  harass_bootstrapping <- c(harass_bootstrapping, harVar$n)
}

t.test(harass_bootstrapping)$conf.int
```

***The interval is between 0.1717028 and 0.1735121, meaning somewhere between 17.2 and 17.4 percent of Americans have been harassed***
...
***It would be a wider interval, with it being five percent less accurate than the 95% interval***
